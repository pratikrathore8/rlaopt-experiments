...
source "${1:-containers/cuda.env}"
if command -v skopeo >/dev/null 2>&1; then
  DIGEST="$(skopeo inspect --override-os linux --override-arch amd64 \
    --format '{{.Digest}}' "docker://${CUDA_BASE_IMAGE}")"
elif command -v docker >/dev/null 2>&1; then
  DIGEST="$(docker manifest inspect --verbose "$CUDA_BASE_IMAGE" | \
...
echo "CUDA_BASE_IMAGE_DIGEST=${DIGEST}"
echo "Immutable reference: ${CUDA_BASE_IMAGE%:*}@${DIGEST}"
