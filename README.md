...
The `manifest` command expands the multinomial grid into one self-contained JSON record per problem, solver, and backend. Feature, target, and solver randomness use independently derived deterministic streams from the recorded master seed; the complete problem specification and resulting problem ID are stored in every job. The smoke grid contains six problem instances and 18 jobs per backend.
