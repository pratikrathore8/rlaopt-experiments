...
def _rlaopt(
    ...
    try:
        solver = PCG(
            LinSys(normal_op, rhs, reg=float(ridge)),
            PCGConfig(preconditioner_config=preconditioner),
        )
    finally:
        ...
        trace.append(
            {
                ...
                "native_relative_residual": relative,
    ...
    return SolveResult(
        estimate.squeeze(-1), time.perf_counter() - started, len(trace), status, trace
    )


def scipy_lsqr(
    problem: RidgeProblem, ridge: float, tolerance: float, max_iters: int, timeout_seconds: float
) -> SolveResult:
    ...
    operator = LinearOperator(
        x.shape, matvec=lambda v: x @ v, rmatvec=lambda v: x.T @ v, dtype=np.float64
    )
    runtime, output = _timed(
        torch.device("cpu"),
        lambda: lsqr(
            operator,
            y,
...
def cuml_lsmr(
    problem: RidgeProblem, ridge: float, tolerance: float, max_iters: int, **_: Any
) -> SolveResult:
    try:
        import cupy as cp
